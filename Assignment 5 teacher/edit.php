...
if(isset($_POST['crud'])){//Get Values from input
  ...
  if(empty($name) || empty($email) || empty($cell) || empty($username) || empty($location) || empty($age) || empty($gender) || empty($department)){
    ...
    connect()->query("UPDATE teachers SET name='$name',email='$email',cell='$cell',username='$username',location='$location', age='$age',gender='$gender',dept='$department',photo='$photo_name' WHERE id='$id'");
  }
}
